    ...
    def build_model(
        ...
        **model_args
        ...
        if optimizer == "Adam":
            ...
            raise Exception("Optimizer %s unknown" % optimizer)

        # Metrics
        out = tf.nn.softmax(y_out, axis=-1)
        acc, acc_update = tf.metrics.accuracy(
            tf.argmax(self.inputs[-1], 1), tf.argmax(out, 1), name="accuracy"
        )
        prec, prec_update = tf.metrics.precision(
            tf.argmax(self.inputs[-1], 1), tf.argmax(out, 1), name="precision"
        )
        recall, recall_update = tf.metrics.recall(
            tf.argmax(self.inputs[-1], 1), tf.argmax(out, 1), name="recall"
        )
        f1 = (2 * (prec * recall)) / (prec + recall)
        auc, auc_update = tf.metrics.auc(
            tf.argmax(self.inputs[-1], 1), tf.argmax(out, 1), name="auc"
        )
