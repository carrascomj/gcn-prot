...
class BaseTrainer(object):
    def __init__(self, output_dir=None):
        ...
        self.output_dir = (
            os.path.expandvars(output_dir) if output_dir is not None else None
        )
        self.summary = {}
    ...
    def write_summary(self, kfold_i=None):
        ...
        if kfold_i is not None:
            history_file = os.path.join(
                self.output_dir, "history_k" + str(kfold_i) + ".csv"
            )
            summary_file = os.path.join(
                self.output_dir, "summary_k" + str(kfold_i) + ".csv"
            )
        else:
            ...
            for i in range(nb_epochs):
                ...
                if early_stop_epochs > 0:
                    if epochs_since_best == early_stop_epochs:
                        self.logger.info(
                            "Stopping training early due to no progress within the last "
                            + str(early_stop_epochs)
                            + " epochs."
                        )
                        break
