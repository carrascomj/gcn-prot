...
def GraphKernels(
    ...
    w = tf.Variable(
        tf.truncated_normal(
            [nb_features, nb_kernels * 2],
            stddev=np.sqrt(2 / ((nb_nodes * nb_features) + (nb_nodes * nb_kernels))),
        )
    )
...
def GraphConv(v, a, nb_filters, activation, training=None, namespace="graphconv_"):
    ...
    w = tf.Variable(
        tf.truncated_normal(
            [nb_features * support, nb_filters],
            stddev=np.sqrt(2 / ((nb_features * support) + (nb_filters))),
        ),
        name=namespace + "w",
    )
    w = tf.tile(tf.expand_dims(w, axis=0), [batch_size, 1, 1])
...
def AverageSeqGraphPool(v, c, pool_size, namespace="averseqgraphpool_"):
    ...
    v_prime = tf.layers.average_pooling1d(
        v, pool_size, pool_size, name=namespace + "vprime"
    )
    c_prime = tf.layers.average_pooling1d(
        c, pool_size, pool_size, name=namespace + "cprime"
    )

    # Generate new A
    a_prime = [
        L2PDist(c_prime, namespace=namespace + "l2pdist_a"),
        CosinePDist(c_prime, namespace=namespace + "cospdist_a"),
    ]
